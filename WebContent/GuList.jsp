...
<html lang="en">
  ...
  <body>
    <nav class="navbar bg-dark navbar-dark main_nav" style="position: fixed">
      <a
        class="navbar-brand"
        href=".\main.jsp"
        ...
        <ul class="navbar-nav">
          <li class="nav-item dropdown">
            ...
            <div class="dropdown-menu">
              <a class="dropdown-item" href="#">강남구 게시판</a>
		...
		<c:choose>
			...
			<c:otherwise>
				...
				<tr align="center">
					...
					<td align="left">
						<a href="../wagleboard/view.do?idx=${row.idx }">${row.title }</a>
					</td>
					<td>${row.name }</td>
					...
					<c:if test="${not empty row.ofile }">
						<!-- 파일 다운로드 시 다운로드 횟수를 증가해야 하므로
							게시물의 일련번호가 필요함. -->
						<a href="../wagleboard/download.do?ofile=${row.ofile }&sfile=${row.sfile}&idx=${row.idx}">[Down]</a>
					</c:if>
					</td>
				</tr>
				</c:forEach>
